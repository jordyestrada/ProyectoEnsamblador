...
update_camera proc
    ...
    ; Centrar cámara en el jugador (corregido)
    mov al, byte ptr [px]
...
hud_col:
    ...
    int 10h
    inc cx
    cmp cx, 320         ; Ancho completo de pantalla
    jl hud_col
    inc dx
    cmp dx, 20
    jl hud_loop
    
    ; Mostrar contadores simples pero visibles
    mov cx, 10
    mov dx, 5
...
draw_hud_char proc
    ...
    ; Dibujar carácter como bloque simple
    mov bx, 0
char_y:
    mov si, 0
char_x:
...
draw_status_message proc
    push ax
    push bx
    push si
    
    ; Dibujar mensaje simple
    mov bx, 0
status_msg_y:
    mov si, 0
status_msg_x:
...
draw_viewport proc
    ...
    ; Simplificar completamente para evitar todos los errores de tipos
...
viewport_x_loop:
    ; Calcular posición Y en el mapa con operaciones más simples
    ...
    ; Calcular índice del mapa: Y * MAPA_ANCHO + X usando operaciones más compatibles
    push bx             ; Guardar X
    mov dx, MAPA_ANCHO
    ...
    jge skip_current_tile
    
    ; Obtener tile del mapa
    mov al, byte ptr [mapa + bx]
    
    ; Calcular posición en pantalla usando operaciones más simples
    push ax             ; Guardar tipo de tile
    
    ; X en pantalla = di * TILE_SIZE
    mov ax, di
    mov dx, TILE_SIZE
    mul dx
    mov cx, ax          ; CX = X en pantalla
    
    ; Y en pantalla = si * TILE_SIZE + 20 (offset HUD)
    mov ax, si
    mov dx, TILE_SIZE
    mul dx
    add ax, 20          ; Offset para HUD
    mov dx, ax          ; DX = Y en pantalla
    
    ; Verificar si está dentro del viewport visible
    cmp cx, 160         ; Límite derecho del viewport
    jge skip_tile_draw
    cmp dx, 116         ; Límite inferior del viewport (20+96)
    jge skip_tile_draw
...
crystal_center_x:
    ...
    pop ax
    ret
draw_crystal_item endp

draw_treasure_item proc
    push ax
    push bx
    push si
...
treasure_center_x:
    ...
    inc si
    cmp si, 10
    jl treasure_center_x
    inc bx
    cmp bx, 10
    jl treasure_center_y
    
    pop si
    pop bx
    pop ax
    ret
draw_treasure_item endp

draw_pit_effects proc
    push ax
    push bx
    push si
    
    ; Borde gris alrededor del pozo
    mov bx, 0
pit_border_y:
    mov si, 0
pit_border_x:
...
draw_player proc
    ...
    ; Calcular posición del jugador en viewport usando operaciones más simples
    mov al, byte ptr [px]
...
draw_player_sprite proc
    ...
    ; Dibujar jugador como figura humanoide mejorada y más visible
    ; Cabeza (más grande)
    mov bx, 1
    mov si, 4
head_loop_y:
